...
const ExcellenceAndProcess = () => {
  return (
    <section className="py-8 md:py-16 bg-brand-cream relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-brand-forest/5 rounded-full blur-3xl -mr-32 -mt-32" />
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-brand-forest/5 rounded-full blur-3xl -ml-32 -mb-32" />

      <div className="container px-4 md:px-6 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-6 md:mb-12">
          <h2 className="text-brand-forest/40 font-serif text-[10px] md:text-sm uppercase tracking-[0.4em] mb-1 font-bold">Our Excellence & Journey</h2>
          <h3 className="text-brand-forest font-serif text-2xl md:text-4xl leading-tight font-bold">Tradition. Science. Results.</h3>
          <div className="w-12 md:w-20 h-1 bg-brand-forest/20 mx-auto rounded-full mt-2" />
        </div>

        {/* 1. WHY CHOOSE US - RESTORED ORIGINAL SHAPE & SIZE */}
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-4 mb-8 md:mb-16">
          {features.map((item, i) => (
            <div key={i} className="bg-brand-forest border border-brand-forest/10 p-3 md:p-5 rounded-2xl md:rounded-[1.5rem] hover:opacity-90 transition-all group flex flex-col items-center text-center shadow-lg">
              <div className="w-9 h-9 md:w-12 md:h-12 bg-white/10 rounded-lg flex items-center justify-center mb-2 md:mb-3 group-hover:scale-110 transition-transform">
                <item.icon className="text-brand-cream w-4 h-4 md:w-6 md:h-6" />
              </div>
              <h4 className="text-brand-cream font-serif text-[11px] md:text-sm font-bold leading-tight">{item.title}</h4>
              <p className="text-brand-gold/60 text-[8px] md:text-[9px] uppercase tracking-widest mt-1 font-bold">{item.subtitle}</p>
            </div>
          ))}
        </div>

        {/* 2. THE JOURNEY - RESTORED ORIGINAL SHAPE & SIZE */}
        <div className="bg-brand-forest border border-brand-gold/20 rounded-[1.5rem] md:rounded-[2.5rem] p-5 md:p-10 relative overflow-hidden shadow-2xl">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 md:gap-10 relative z-10">
            <div className="lg:max-w-[200px] w-full border-b lg:border-b-0 lg:border-r border-brand-gold/20 pb-4 lg:pb-0 lg:pr-6">
              ...
              <p className="text-brand-cream/40 text-[10px] md:text-xs leading-relaxed uppercase tracking-widest">Personalized Vedic protocol.</p>
            </div>
              ...
              {steps.map((step, idx) => (
                <div key={idx} className="relative flex items-center lg:flex-col lg:text-center gap-3 group">
                  <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-brand-gold flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 transition-transform">
                    <step.icon className="text-brand-forest w-5 h-5 md:w-7 md:h-7" />
                  </div>
                  <div>
                    ...
                    <p className="text-brand-cream/40 text-[9px] md:text-[11px] leading-tight uppercase font-medium">{step.desc}</p>
                  </div>
                  {idx < steps.length - 1 && (
