...
const AboutPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-brand-cream font-sans selection:bg-brand-gold selection:text-brand-black">
      <Seo title="About Us - Ayurveda Veda" description="Learn about our legacy, values, and expert Vaidyas." />
      <Navbar />
      
      <main className="flex-1 pt-20">
        {/* Enhanced About Hero */}
        <div className="relative bg-brand-forest py-20 md:py-32 overflow-hidden">
          ...
          <div className="container px-4 md:px-6 relative z-10 text-center">
            <div className="inline-block px-4 py-1 mb-6 border border-brand-gold/30 rounded-full bg-brand-gold/5 backdrop-blur-sm">
              <span className="text-brand-gold text-xs md:text-sm font-medium tracking-widest uppercase">Est. 1958 • Legacy of Healing</span>
            </div>
            <h1 className="text-brand-gold font-serif text-4xl md:text-7xl lg:text-8xl mb-6">Our Legacy</h1>
            <p className="text-brand-cream/80 text-lg md:text-2xl max-w-3xl mx-auto leading-relaxed font-light">
        ...
        {/* Team Section (Removed duplicate header) */}
        <Team />

        {/* Professional Social Connect Banner */}
        <div className="bg-brand-forest py-16 md:py-24 relative overflow-hidden">
          ...
          <div className="container px-4 md:px-6 relative z-10 text-center">
            <h2 className="text-brand-gold font-serif text-sm md:text-lg uppercase tracking-[0.4em] mb-6 font-bold">Join Our Community</h2>
            <h3 className="text-brand-cream font-serif text-3xl md:text-6xl mb-10 md:mb-12">Healing together, sharing wisdom.</h3>
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6">
              {[
                ...
                <a
                  ...
                  className={cn(
                    "flex items-center gap-4 px-6 md:px-8 py-3 md:py-4 rounded-2xl border border-brand-gold/20 bg-white/5 text-brand-gold backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 group shadow-2xl",
                    social.color
                  )}
                >
                  <social.icon className="w-5 h-5 md:w-6 md:h-6 transition-transform group-hover:scale-110" />
                  <span className="font-bold text-[10px] md:text-sm uppercase tracking-widest">{social.name}</span>
                </a>
              ))}
            </div>

            <div className="mt-12 md:mt-16 pt-12 md:pt-16 border-t border-brand-gold/10">
              <p className="text-brand-cream/40 text-[11px] md:text-sm max-w-2xl mx-auto italic leading-relaxed">
